...
def check_flow(openlaneDir, tileName, interruptFlow, logger, olRun):
    if os.path.exists(
            f"{openlaneDir}/designs/{tileName}/runs/{olRun}/errors.log"):
        with open(f"{openlaneDir}/designs/{tileName}/runs/{olRun}/errors.log",
                  "r") as f:
            ...
            f"{openlaneDir}/designs/{tileName}/runs/{olRun}/warnings.log"):
        with open(
                f"{openlaneDir}/designs/{tileName}/runs/{olRun}/warnings.log",
                "r") as f:
            datafile = f.readlines()
...
def fixUpdateOrder(combinDict, initCombinDict, prevTilePinOrderChange,
                   resizeDim, ioSide, isSuperTile, cornerPos, lastTermLoop):
    pinOrderChangeTemp = {}
    pinOrderChange = {}
    fixedCombinDict = {}
    prevTile = {}
    ...
    if ((isSuperTile and ioSide) or lastTermLoop
            or ioSide in ["Right", "Left"]):
